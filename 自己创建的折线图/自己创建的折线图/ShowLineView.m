...
- (void)drawRate {
    ...
    for (int i = 0; i < self.points.count; i++) {
        ...
        if (i != self.points.count - 1 && self.points.count > 2 ) {
            ...
            nextY = ((ViewHeight / (self.YEndNum - self.YStartNum)) * (nextH - self.YEndNum)) * -1;
        }
        pos_y = ((ViewHeight / (self.YEndNum - self.YStartNum)) * (h - self.YEndNum)) * -1;
        ...
        [tempStr drawInRect:CGRectMake(pos_x - 15, Y, 30, 20) withAttributes:@{NSForegroundColorAttributeName:[UIColor blackColor],NSFontAttributeName:[UIFont systemFontOfSize:13]}];
        pos_x -= itemWidth;
    }
    // 线的颜色
    [[UIColor cyanColor]setStroke];
